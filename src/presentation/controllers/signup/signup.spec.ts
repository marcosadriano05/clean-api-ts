...
describe('SignUp Controller', () => {
  ...
  test('Should return status 400 passwordConfirmation do not matches', async () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'any_name',
  ...
  test('Should return status 201 if valid data is provided', async () => {
    const { sut } = makeSut()
    const httpResponse = await sut.handle(makeFakeRequest())
    expect(httpResponse).toEqual(created(makeFakeAccount()))
  })

  test('Should call Validation with correct param', async () => {
  ...
  test('Should return status 400 if Validation returns an error', async () => {
    const { sut, validationStub } = makeSut()
    jest.spyOn(validationStub, 'validate').mockImplementationOnce(() => new MissingParamError('any_error'))
    const httpResponse = await sut.handle(makeFakeRequest())
